...
Approach.Displacer = function (config = {}) {
  ...
  $elf.config = {
    ...
    how: {
      ...
      drag_select_class: 'displacing', // class to that is added to the where container when dragged over
    },
  ...
  let dispatch = {
    displaceable: {
      ...
      dragstart: (e) => {
        ...
        $elf.call.start(e, $elf);
        if ($elf.config.how.remain_in_source == false) {
          setTimeout(() => {
            displaceable.hide();
          }, 0);
        }
    ...
    where: {
      ...
      dragover: (e) => {
        ...
        let closest_displaceable = $(e.target).closest(
          $elf.config.what,
          e.data.where
        );
      ...
      drop: (e) => {
        ...
        if($elf.config.how.remain_in_source){
          $elf.state.displaceableCopy.remove();
        }
        else{
...
function chooseDropSpot(
  ...
  if (insert_target.neighbor === container) {
    if (insert_target.direction === 0) container.prepend(preview);
    else container.append(preview);
...
function resolveMouseNeighbor(mouse, prev, next) {
  ...
  if (prev_dist < next_dist || (prev_dist === next_dist && prev !== next))
    return {
      dist: prev_dist,
      neighbor: prev,
      direction: 0, // backward
    };
  else if (prev_dist === next_dist && prev === next)
    return {
      dist: prev_dist,
      neighbor: prev,
...
function pointToBoxDistance2(
  ...
  corner.TL = pyth_dist(
    numeric_difference(box.left, p.x),
    numeric_difference(box.top, p.y)
  ); //top left
  corner.BL = pyth_dist(
    numeric_difference(box.left, p.x),
    numeric_difference(box.bottom, p.y)
  ); //bottom left
  corner.TR = pyth_dist(
    numeric_difference(box.right, p.x),
    numeric_difference(box.top, p.y)
  ); //top right
  corner.BR = pyth_dist(
    numeric_difference(box.right, p.x),
